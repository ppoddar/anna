...
describe('Tests for Order Managemnent Service', function () {
    ...
    */
    ...
    it('user: valid login', function (done) {
    ...
    */
    ...
    it('user: invalid login', function(done) {
        const unknown_user = {id:'who', password:'anything'}
		var basicAuth = `Basic ${Buffer.from(`${unknown_user.id}:${unknown_user.password}`).toString('base64')}`
        request.post({
            headers:{Authorization: basicAuth},
            url: `${BASE_URL}/user/login`
        }, function(err,res,body) {
            ...
            assert.property(errorResponse, 'message')
            assert.equal(errorResponse.message.id, 'user-not-found')
            assert.include(errorResponse.message.variables, unknown_user.id)
